...
ggplot(data_num) +
  ...
  scale_x_continuous(breaks = pretty_breaks(n = 16), minor_breaks = seq(6000000, 14000000, by = 500000), labels = label_number(suffix = "", scale = 1e-6)) +
  labs(title = "Box blot showing total length (>= 1000 bp)", x = "Total length (in millions)") +
  theme(axis.text.x = element_text(angle = 60, hjust = 1, size=10), axis.text.y = element_blank(), axis.ticks.y = element_blank(), axis.title = element_text(size=12), plot.title = element_text(size=15))
```
...
ggplot(data_pivot, aes(metric, value)) +
  ...
  theme(axis.text.x = element_text(angle = 60, hjust = 1, size=10), axis.text.y = element_text(size=10), axis.title = element_text(size=12), plot.title = element_text(size=15)) + 
